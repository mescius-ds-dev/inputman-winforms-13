...
public partial class Validator : InputManWin13_Demo.DemoBase_settings
{
    ...
    {
        // ***** 初期設定
        gcListBox1.ItemCheck += (s, e) => SetValidateAction(e);
        checkBox1.CheckedChanged += (s, e) => SetSupportRealTimeValidating();

        // ***** 初期値
        gcListBox1.Items[5].Checked = true;
    }

    private void SetValidateAction(ItemCheckEventArgs e)
    {
        // 検証アクションのリセット
    ...
    private void SetSupportRealTimeValidating()
    {
        // リアルタイム検証
    ...
    private ValidateActionMethod GetAction(int index) => index switch
    {
        // GcBalloonTipによる通知(GcBalloonTipNotify)
        0 => GetGcBalloonTipNotify,

        // ３段階のボーダーによる通知(ThreeStateBorderNotify)
        1 => GetThreeStateBorderNotify,

        // ３段階のアイコンによる通知(ThreeStateIconNotify)
        2 => GetThreeStateIconNotify,
        ...
        10 => GetGcBalloonTipNotifyWinStyle,
